...
def detrend_robustness():
    ...
    # level/cycle correlations should all be materially positive; assert the finding is not trend-only
    assert min(R(lin, g), R(cyc, g)) > 0.3, "concentration r collapses once detrended -> was a shared trend"
...
def lp_bands():
    ...
    for name in ["goods", "services", "headline"]:
        r = pd.read_csv(f"outputs/figures/irf_results_dwell_{name}.csv")
        row = r[r.h == 8].iloc[0]
...
if __name__ == "__main__":
    corr_test(); detrend_robustness(); segmentation_crosscheck(); lp_bands(); reform_did_inference()
